# Александр Долженков
Веб-разработчик | Fullstack-разработчик

E-mail: [ra3wok@mail.ru](mailto:ra3wok@mail.ru)\
GitHub: [github.com/okb3wok](https://github.com/okb3wok)\
Telegram: [ra3wok](https://t.me/ra3wok)

## ОБО МНЕ
Веб-разработчик полного цикла: cделаю фронтэнд, бэкэнд, установлю на linux сервер, настрою кэширование. Есть опыт написания телеграм чат-ботов. Мои основные языки программирования: JavaScript, Python и PHP.
Умею работать с современными инструментами разработки (PhpStorm,PyCharm). Использую ИИ-помощника Codeium. Готов решать сложные задачи, создавать эффективные веб-приложения
и удобные пользовательские интерфейсы под нужды бизнеса.


## ТЕХНИЧЕСКИЕ НАВЫКИ

### Frontend:
- JavaScript
- HTML / CSS
- React
- Redux
- Webpack
- Node.js
- Bootstrap
- Ant Design

### Backend:

- PHP
- Django
- Python
- Node.js
- Express
- Doctrine
- Redis
- MySQL, 
- PostgreSQL
- Swagger
- Flask
- FastAPI

  
### Инструменты:
- PhpStorm
- PyCharm
- Codeium
- Git

### Инфраструктура и CI/CD
Linux, nginx, github actions

## ПРОЕКТЫ

### aviaautomatica.ru
[aviaautomatica.ru](https://aviaautomatica.ru),   
*Технологии:* Django, Redis, PostgreSQL, TailwindCSS, Webpack  
*Краткое описание:* Разработка сайта для компании АО "Авиаавтоматика" им. В.В. Тарасова"


### bus-max.ru
[bus-max.ru](https://bus-max.ru),  
*Технологии:* React, Redux, JS, PHP, MySQL, Webpack, Wordpress, Bootstrap, AdminLTE  
*Краткое описание:* Система продажи билетов. Интеграция эквайринга от Тинькофф

### Cайты - визитки
*Примеры:* [art-komnata](https://github.com/okb3wok/art-komnata.ru), 
[atlaskursk.ru](https://github.com/okb3wok/atlas)  
*Технологии:* Webpack, SCSS, JS, Twig, PHP, MySQL  
*Краткое описание:* Сайты-визитки для мебельной компании и дизайн студии.

### Телеграм чат-бот
*Пример:* [simple-telegram-bot](https://github.com/okb3wok/simple-telegram-bot),   
*Технологии:* HTML, CSS, Python, pyTelegramBotAPI, FastAPI, uvicorn, nginx  
*Краткое описание:* Простой телеграм бот для поиска контактов сотрудников в exel файле.

## Опыт

- 2022 - н.в. Инженер-программист в АО "Авиаавтоматика" (Курск)

- 2012 - н.в. Фриланс. Коммерческая разработка сайтов.

- 2008 - 2012 Исследовательская работа в ЮЗГУ

## ОБРАЗОВАНИЕ

В 2008 году получил степень магистра техники и технологии по
направлению "Проектирование и технология электронных средств" 
в Юго-западном государственном университете (ЮЗГУ)

В 2012 закончил аспирантуру ЮЗГУ по специальности 05.13.06. 
"Автоматизация и управление технологическими процессами и производствами"


## ЯЗЫКИ

English Intermediate


